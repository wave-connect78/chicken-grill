<?php
    ...
    \Stripe\Stripe::setApiKey(STRIPE_ASNIERES_API_KEY);
                    ...
                    if(isset($_SESSION['sale_price'])){
                        if(isset($_SESSION['codepromo_update'])){
                            executeQuery("UPDATE manage_code_promo SET nb = :nb WHERE code_name =:code_name AND user_id =:user_id",array(
                                ...
                                insertCommandeAndValidatePayment($transactionID,$commande_code,$commande_detail,$somme,$paidAmount,$payment_status,$paidCurrency,'Asnieres',$tel,$email);
                        }else{
...
<script>
    ...
    let cvcNumber = false;
    let cExpiry = false;
    let stripe = Stripe('<?php echo STRIPE_ASNIERES_PUBLIC_KEY; ?>');
