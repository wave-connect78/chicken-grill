            ...
            if(isset($_POST['codepromo'])){
                ...
                if($resultat->rowCount() > 0){
                    ...
                    $result = executeQuery("SELECT * FROM manage_code_promo WHERE user_id = :user_id AND code_name = :code_name",array(
                     ...
                     $message = '<div class="error">Votre code promo n\'est pas valide. Assurez vous que votre code promo est valide pour le restaurant Chicken grill Saint-denis</div>';
                }
            } else {
                foreach ($_SESSION['cart'] as $key => $value) {
